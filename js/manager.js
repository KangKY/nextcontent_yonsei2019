...
$(document).ready(function() {
  ...
  $(".modal-overlay").click(function() {
    if ($(".modal.active").attr("id") == "modalGraph") return;
...
function ListInit(dataModule, dataProperty) {
  ...
  ref.once("value").then(function(snapshot) {
    ...
    if (snapshot.val().Count > 0) {
      ...
      avgProperty = [
        avgProperties.content,
        avgProperties.space,
        avgProperties.manual,
        avgProperties.expression
    ...
    var middleOverModule = [];

    for (var i = 0; i < moduleText.length; i++) {
      var mPoint = dataModule[i];
      moduleList.push(new Modules(moduleText[i], 0, 0, mPoint));
      if (mPoint <= 20) consModule.push(moduleText[i]);
      else if (mPoint >= 80) prosModule.push(moduleText[i]);

      if (mPoint >= 50) middleOverModule.push(moduleText[i]);
    }
    ...
    for (var i = 0; i < propertyText.length; i++) {
      ...
      if (mPoint <= 20) consProperty.push(propertyText[i]);
      else if (mPoint >= 80) prosProperty.push(propertyText[i]);

      if (mPoint >= 50) middleOverProperty.push(propertyText[i]);
    }
    ...
    $.getJSON("./js/module.json", function(data) {
      ...
      for (var i = 0; i < _module.length; i++) {
        if (
          JSON.stringify(middleOverModule.sort()) ==
          JSON.stringify(_module[i].module.sort())
        ) {
          $("#module .description .description_title").text(
            _module[i].result_title
          );
          $("#module .description .description_desc").text(
            _module[i].result_desc
          );
          if (consModule.length > 0) {
            var _text =
              "<br>특히, " +
              consModule.join(", ") +
              "을(를) 중심으로 콘텐츠를 향상시키는 것을 권장한다.";
            $("#module .description .description_desc").append(_text);
          }
    ...
    $.getJSON("./js/property.json", function(data) {
      ...
      for (var i = 0; i < _property.length; i++) {
        if (
          JSON.stringify(middleOverProperty.sort()) ==
          JSON.stringify(_property[i].property.sort())
        ) {
          $("#property .description .description_title").text(
            _property[i].result_title
          );
          $("#property .description .description_desc").text(
            _property[i].result_desc
          );
    ...
    $("#module .pros-cons.pros span").text(
      prosModule.join(", ") == "" ? "없음" : prosModule.join(", ")
    );
    $("#module .pros-cons.cons span").text(
      consModule.join(", ") == "" ? "없음" : consModule.join(", ")
    );
    $("#property .pros-cons.pros span").text(
      prosProperty.join(", ") == "" ? "없음" : prosProperty.join(", ")
    );
    $("#property .pros-cons.cons span").text(
      consProperty.join(", ") == "" ? "없음" : consProperty.join(", ")
    );
      ...
      if ($.inArray(data.key, content) != -1) {
        ...
        $.each(data.val(), function(index, item) {
          ...
          var template = $("#cardTemplate")
            ...
            .clone();
          template
            .attr("data-type", "content")
            .attr("data-category", questionItem.category);
...
function getTranlateTimestamp(regiDates) {
  var result = regiDates.map((v, idx) => {
    ...
    return (
      _date.getMonth() + 1 + "월" + _date.getDate() + "일(" + (idx + 1) + "차)"
    );
  });
  return result;
}
...
function drawHistoryTabs(timestamps, historyText) {
  ...
  $(".custom-tabs .custom-tab a").on("click", function() {
    var a =
      $(".custom-tabs span:first-child a").offset().left +
      $(".custom-tabs span:first-child a").outerWidth() / 2;
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      var ref = firebase
        ...
        .child("Result");
      ref
        .orderByChild("RegiDate")
        .equalTo(RegiDate)
        .once("value")
        .then(function(snapshot) {
          $(".history-top-menu span.submenu[data-panel='module']").trigger(
            "click"
          );
          //console.log(snapshot.val());
          snapshot.forEach(function(childSnapshot) {
...
function generateModulePoints() {
  $("#cardList .card.custom.selected").each(function() {
    ...
    var pobj = sendPropertyList.find(
      x =>
        x.mTitle ==
        $(this)
          .parent()
  ...
  $("#cardList .card.custom").each(function() {
    try {
      ...
      //console.log(mobj);
      mobj.mAnswerCnt++;
      if ($(this).hasClass("selected")) mobj.mYesCnt++;
    } catch (e) {
      console.log(
        $(this)
          .parent()
  ...
  for (var i = 0; i < sendModuleList.length; i++) {
    if (sendModuleList[i].mAnswerCnt != 0)
      sendModuleList[i].mPoint = parseInt(
        (sendModuleList[i].mYesCnt * 100) / sendModuleList[i].mAnswerCnt
      );
  }

  for (var j = 0; j < sendPropertyList.length; j++) {
    if (sendPropertyList[j].mAnswerCnt != 0)
      sendPropertyList[j].mPoint = parseInt(
        (sendPropertyList[j].mYesCnt * 100) / sendPropertyList[j].mAnswerCnt
      );
  }
  console.log(sendModuleList);
...
function setResultDataToDB() {
  ...
  ref.once("value").then(function(snapshot) {
    ...
    if (avgCount != 0) {
      for (var key in avgProperties) {
        ...
        avgProperties[key] =
          (sendProperty[key] + parseInt(avgProperties[key])) / (avgCount + 1);
      }
      for (var key in avgModules) {
        //console.log( key + '=>' + avgModules[key] );
        avgModules[key] =
          (sendModule[key] + parseInt(avgModules[key])) / (avgCount + 1);
      }
  ...
  if (user != null) {
    var resultListRef = firebase
      .database()
      .ref("users/" + user.uid)
      .child("Result");
    resultListRef.push(resultObj, function(error) {
      if (error) {
        // The write failed...
        console.log(error);
      } else {
    ...
    $.each(items.content, function(idx, item) {
      ...
      $.each(items.space, function(idx, item) {
        ...
        $.each(items.age, function(idx, item) {
          ...
          var guideTemplate = $("#guideTemplate")
            .children()
            .clone();

          guideTemplate.find("div.guideline").text(resultData.guideline);
          for (var i = 0; i < resultData.recommend_title.length; i++) {
            var recommendTemplate = $("#recommendTemplate")
              .children()
              .clone();
            recommendTemplate
              .find("div.guide-title")
              .text(resultData.recommend_title[i]);
            recommendTemplate
              .find("div.guide-desc")
              .text(resultData.recomment_desc[i]);

            for (var j = 0; j < resultData.image.length; j++) {
              if (resultData.recommend_title.length > 1 && i == 0) break;
              recommendTemplate
                .find("div.guide-image img")
                .attr(
                  "src",
                  "img/guide/" + category + "/" + resultData.image[j]
                );
              recommendTemplate
                .find("div.guide-image span.source")
                .text(resultData.source[j]);
            }
...
function setGuideSelections() {
  ...
  if ($("#content-body").find("a.btn.selected").length != 0)
    $("#content-body")
      .find("a.btn.selected")
      .each(function() {
        content.push($(this).data("category"));
      });
  else
    $("#content-body")
      .find("a.btn")
  ...
  Highcharts.chart("moduleChart", {
    ...
    xAxis: {
      ...
      labels: {
        formatter: function() {
          if (
            this.pos < moduleList.length &&
            moduleList[this.pos].underScore()
          ) {
            return '<span style="fill: #ed4733;">' + this.value + "</span>";
          } else if (
            this.pos < moduleList.length &&
            moduleList[this.pos].overScore()
          ) {
  ...
  Highcharts.chart("propertyChart", {
    ...
    xAxis: {
      ...
      labels: {
        formatter: function() {
          if (
            this.pos < propertyList.length &&
            propertyList[this.pos].underScore()
          ) {
            return '<span style="fill: #ed4733;">' + this.value + "</span>";
          } else if (
            this.pos < propertyList.length &&
            propertyList[this.pos].overScore()
          ) {
            return '<span style="fill: #1117d0;">' + this.value + "</span>";
          } else {
            return this.value;
